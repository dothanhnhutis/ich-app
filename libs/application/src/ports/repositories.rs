...
pub trait UserRepository: Send + Sync {
    ...
    fn list(
        &self,
        filter: UserFilter,
    ) -> impl Future<Output = Result<(Vec<User>, i64), AppError>> + Send;
    ...
    fn soft_delete(&self, id: Uuid) -> impl Future<Output = Result<(), AppError>> + Send;
...
pub trait RoleRepository: Send + Sync {
    ...
    fn find_all_permissions(
        &self,
    ) -> impl Future<Output = Result<Vec<Permission>, AppError>> + Send;
    ...
    fn create_with_permissions(
        ...
        new_role: NewRole,
        permission_ids: &[Uuid],
    ) -> impl Future<Output = Result<Role, AppError>> + Send;

    /// Tìm vai trò theo id (chỉ role chưa xoá mềm).
    fn find_by_id(
        &self,
        id: Uuid,
    ) -> impl Future<Output = Result<Option<Role>, AppError>> + Send;
    ...
    fn find_roles_for_user(
        &self,
        user_id: Uuid,
    ) -> impl Future<Output = Result<Vec<Role>, AppError>> + Send;

    /// Danh sách vai trò (lọc + phân trang); trả về (items, tổng số khớp).
    fn list(
        &self,
        filter: RoleFilter,
    ) -> impl Future<Output = Result<(Vec<Role>, i64), AppError>> + Send;
...
pub trait LocationRepository: Send + Sync {
    ...
    ) -> impl Future<Output = Result<Location, AppError>> + Send;

    /// Tìm kho theo id (chỉ kho chưa xoá mềm).
    fn find_by_id(
        &self,
        id: Uuid,
    ) -> impl Future<Output = Result<Option<Location>, AppError>> + Send;

    /// Danh sách kho (lọc + phân trang + sắp xếp); trả về (items, tổng số khớp).
    fn list(
        &self,
        filter: LocationFilter,
    ) -> impl Future<Output = Result<(Vec<Location>, i64), AppError>> + Send;
...
pub trait ZoneRepository: Send + Sync {
    ...
    fn create(
        &self,
        new_zone: NewZone,
    ) -> impl Future<Output = Result<Zone, AppError>> + Send;

    /// Tìm khu vực theo id (chỉ zone chưa xoá mềm).
    fn find_by_id(
        &self,
        id: Uuid,
    ) -> impl Future<Output = Result<Option<Zone>, AppError>> + Send;

    /// Danh sách khu vực (lọc + phân trang + sắp xếp); trả về (items, tổng số khớp).
    fn list(
        &self,
        filter: ZoneFilter,
    ) -> impl Future<Output = Result<(Vec<Zone>, i64), AppError>> + Send;
...
pub trait BinRepository: Send + Sync {
    /// Tạo kệ mới.
    fn create(&self, new_bin: NewBin) -> impl Future<Output = Result<Bin, AppError>> + Send;

    /// Tìm kệ theo id (chỉ bin chưa xoá mềm).
    fn find_by_id(
        &self,
        id: Uuid,
    ) -> impl Future<Output = Result<Option<Bin>, AppError>> + Send;

    /// Danh sách kệ (lọc + phân trang + sắp xếp); trả về (items, tổng số khớp).
    fn list(
        &self,
        filter: BinFilter,
    ) -> impl Future<Output = Result<(Vec<Bin>, i64), AppError>> + Send;
...
pub trait VendorRepository: Send + Sync {
    ...
    ) -> impl Future<Output = Result<Vendor, AppError>> + Send;

    /// Tìm nhà cung cấp theo id (chỉ vendor chưa xoá mềm).
    fn find_by_id(
        &self,
        id: Uuid,
    ) -> impl Future<Output = Result<Option<Vendor>, AppError>> + Send;

    /// Danh sách nhà cung cấp (lọc + phân trang + sắp xếp); trả về (items, tổng số khớp).
    fn list(
        &self,
        filter: VendorFilter,
    ) -> impl Future<Output = Result<(Vec<Vendor>, i64), AppError>> + Send;
...
pub trait ItemRepository: Send + Sync {
    /// Tạo vật tư mới.
    fn create(&self, new_item: NewItem) -> impl Future<Output = Result<Item, AppError>> + Send;

    /// Tìm vật tư theo id (chỉ item chưa xoá mềm).
    fn find_by_id(
        &self,
        id: Uuid,
    ) -> impl Future<Output = Result<Option<Item>, AppError>> + Send;

    /// Danh sách vật tư (lọc + phân trang + sắp xếp); trả về (items, tổng số khớp).
    fn list(
        &self,
        filter: ItemFilter,
    ) -> impl Future<Output = Result<(Vec<Item>, i64), AppError>> + Send;
...
pub trait BomRepository: Send + Sync {
    ...
    fn create_with_lines(
        ...
        new_bom: NewBom,
        lines: &[NewBomLine],
    ) -> impl Future<Output = Result<(Bom, Vec<BomLine>), AppError>> + Send;

    /// Tìm BOM header theo id (chỉ bom chưa xoá mềm).
    fn find_by_id(&self, id: Uuid) -> impl Future<Output = Result<Option<Bom>, AppError>> + Send;

    /// Tìm BOM kèm dòng (nested read); None nếu bom không tồn tại / đã xoá.
    fn find_with_lines(
        &self,
        id: Uuid,
    ) -> impl Future<Output = Result<Option<(Bom, Vec<BomLine>)>, AppError>> + Send;

    /// Danh sách BOM (lọc + phân trang + sắp xếp); trả về (items, tổng số khớp).
    fn list(
        &self,
        filter: BomFilter,
    ) -> impl Future<Output = Result<(Vec<Bom>, i64), AppError>> + Send;
    ...
    fn update(
        ...
        id: Uuid,
        changes: BomUpdate,
    ) -> impl Future<Output = Result<Option<Bom>, AppError>> + Send;
    ...
    fn add_line(
        ...
        bom_id: Uuid,
        new_line: NewBomLine,
    ) -> impl Future<Output = Result<BomLine, AppError>> + Send;
    ...
    ) -> impl Future<Output = Result<Option<BomLine>, AppError>> + Send;
    ...
    fn list_lines(
        &self,
        bom_id: Uuid,
    ) -> impl Future<Output = Result<Vec<BomLine>, AppError>> + Send;
...
pub trait PasswordTokenRepository: Send + Sync {
    ...
    ) -> impl Future<Output = Result<PasswordToken, AppError>> + Send;

    /// Token còn hiệu lực theo hash: chưa dùng (used_at IS NULL) và chưa hết hạn.
    fn find_active_by_hash(
        &self,
        token_hash: &str,
    ) -> impl Future<Output = Result<Option<PasswordToken>, AppError>> + Send;
